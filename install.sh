...
function create-certs() {
  ...
  for d in `echo $services | awk '{print $0}'`; do
    #statements
    cp -R "$(pwd)/$docker_img_tag/$target_dir" "$(pwd)/$d/src/main/java/example"
  done
...
function remove-certs() {
  ...
  for d in `echo $services | awk '{print $0}'`; do
    #statements
    rm -Rf "$(pwd)/$d/src/main/java/example/$target_dir"
  done
...
function start-up(){
    ...
    sudo docker run -ti --rm -p "443:8443" $docker_img_name
